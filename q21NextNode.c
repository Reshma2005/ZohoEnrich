/*Search a number in a given linked list and print the details of the node that lies next to the matched node.*/
...
void search(struct node *h)
{
	...
	while(1)
	{
		...
		if(new->data == key)
		{
			...
			new = new->next;
			printf("The data present next to matched node: %d",new->data);
			return;
		}
		new = new->next;
		count++;
